...
class Trainer(object):
    def __init__(self,
        ...
        #     min_lr=min_lr,
        #     verbose=True)
        # Replace scheduler1 and scheduler2 with this single robust scheduler
        self.scheduler = th.optim.lr_scheduler.CosineAnnealingWarmRestarts(
            self.optimizer, 
            T_0=10,       # Initial cycle length (10 epochs)
            T_mult=2,     # Doubles the cycle length after each restart (10, 20, 40, 80...)
            eta_min=1e-6  # The lowest the LR can go before bouncing back up
        )
    ...
    def create_optimizer(self, optimizer, kwargs, state=None):
        ...
        # add fused = True in kwargs to speed up training 
